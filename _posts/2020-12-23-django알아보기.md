---
title : "[프로그래머스인공지능스쿨]Web Application with Django - django"
...
```
<br>
다시 `python3 manage.py runserver`로 admin페이지에 들어가 로그인을 해보면 잘 된다!  
...
settings.py의 TEMPLATES의 'DIRS'에 이를 명시해줄 수 있는데,  
...
```
이제 my_num을 index.html에서 사용할 것인데, 이를 사용하기 위해 장고 템플릿 언어를 사용한다. 장고 템플릿 언어는 html에서 부가적으로 사용할 수 있는 언어인데, {{}}를 이용해 쓴다. {{}}를 템플릿 변수라고 한다.  
...
`{{ my_name|length }}`와 같이 템플릿 필터를 적용할 수 있다. 이 경우, my_name의 길이를 반환해준다.  
index.html의 템플릿 변수를 이처럼 수정해주면 결과는 다음과 같다.  
![png](/assets/images/2020-12-23/5.png)  
Michael의 문자열 길이가 7이므로 결과는 7로 나온다.  
<br>

`{{ my_name|upper }}`는 my_name에 대해서 모두 대문자로 출력해주는 필터이다.  
...
`{% if element|divisibleby:"2" %}`와 같이 쓰면 element가 숫자 2로 나눠 떨어지게하는 조건을 쓸 수 있다.  
